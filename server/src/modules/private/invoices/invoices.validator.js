...
const createInvoiceValidators = [
    ...
    // validating the invoiceNumber field
    body("invoiceNumber")
        .notEmpty()
        .withMessage("Invoice number is required")
        .isString(),

    // validating the invoiceDate field
    body("invoiceDate")
        .notEmpty()
    ...
    // validating the items array
    body("items")
        .isArray({ min: 1 })
        .withMessage("Items must be a non-empty array"),

    // validating the productId field inside items array
    ...
    // validating the quantity field inside items array
    body("items.*.quantity")
        .notEmpty()
    ...
    // validating the unitPrice field inside items array
    ...
    // validating the taxId field inside items array
    body("items.*.taxId")
        .optional()
        .custom((value) => value === null || mongoose.Types.ObjectId.isValid(value))
        .withMessage("Invalid Tax ID"),

    // validating the discountAmount field inside items array
...
const approveInvoiceValidators = [
    ...
    // validating the invoiceId param
    param("invoiceId")
        .notEmpty()
